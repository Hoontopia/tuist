...
normalize_version() {
    ...
    hyphen_count="$(grep -o '-' <<<"$version" | wc -l | tr -d '[:space:]')"
...
main() {
    ...
    if [[ "$#" -eq 1 ]]; then
        purge_package "$swift_package_root" "$metadata_package_root"
    else
        purge_version "$swift_package_root" "${metadata_package_root}/index.json" "$2"
    fi
